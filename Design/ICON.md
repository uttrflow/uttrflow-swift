...
`uttrflow-logo-original.png` is kept only as heritage: it is the खि the app carried
before this mark, generated by the OpenAI image service (its embedded C2PA manifest
records `digitalSourceType: trainedAlgorithmicMedia`), and nothing builds from it.
