...
notice it, along with the two `doubtful-word-…` cases and their pair. Sequence lists,
paragraph breaks and restatement corrections still have no case, so the first step for
each is a case, not a prompt line. `Docs/bakeoff.md` explains why: a prompt line that is not
