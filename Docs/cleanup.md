...
| Stammers — the same short word twice | "the the deployment" → "the deployment" | ✅ `StammersPass` (≤4 letters; a long repeat is emphasis). A double the language itself makes is kept — "had had", "that that", "bye bye", "no no", "so so" |
...
| Self-correction by restatement | a frame of function words said twice over, each time with a different word after it | "as a gift as a present" → "as a present"; "on tuesday on wednesday" → "on wednesday" | ✅ `Restatement`, beside the trigger rule, so `SelfCorrectionPass` and `PieceJoiner` both do it. The frame is one to three function words opening on a preposition, holding no subject pronoun or auxiliary, with a different content word after each copy and no punctuation inside the half taken back. "To" never opens a frame, being the mark of an infinitive ("we need to wait to finish"); nor does a lone "as", which pairs with itself in a comparison ("as soon as possible"). A frame that opens a clause is not a correction: "I like tea I like coffee both are fine" survives whole. A set phrase of the same shape is the known cost — "for better for worse" loses its first half |
