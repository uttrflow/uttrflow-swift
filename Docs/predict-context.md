...
|---|---|---|---|
| G1 Read | `surroundings` (title + visible runs, 60 ms budget, cached), `PredictStore.recent(in:limit:)`, both carried in `GenerationSituation` | Reader+System, Snapshot, PredictStore, CandidateGeneration, Coordinator.generate | AX fixture tests for run collection and budget; store test for recency/distinctness |
