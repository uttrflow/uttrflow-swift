...
public struct Situation: Sendable, Hashable, Codable {
    ...
    public init(
        ...
        self.branch = Situation.normalised(branch)
        self.connection = Situation.normalised(connection)
        self.environment = environment
        self.file = Situation.normalised(file)
    }

    /// The situation that knows nothing, which is what an unrecognisable window amounts to.
    public static let unknown = Situation()
    ...
    public func similarity(to other: Situation) -> Double {
        let facets = [
            (Situation.agreement(branch, other.branch), Situation.branchWeight),
            (Situation.agreement(connection, other.connection), Situation.connectionWeight),
            (Situation.agreement(environment, other.environment), Situation.environmentWeight),
            (Situation.agreement(file, other.file), Situation.fileWeight),
        ]
        let comparable = facets.compactMap { agreement, weight in agreement.map { ($0, weight) } }
        let total = comparable.reduce(0) { $0 + $1.1 }
        guard total > 0 else { return Situation.unknownAgreement }
    ...
    public func completed(by other: Situation) -> Situation {
        Situation(
    ...
    static func normalised(_ facet: String?) -> String? {
        ...
        let cleaned = trimmedWhitespace(facet).lowercased()
        return cleaned.isEmpty ? nil : cleaned
    }
}

/// A string without its surrounding whitespace, this module having no Foundation to ask.
func trimmedWhitespace(_ text: some StringProtocol) -> String {
