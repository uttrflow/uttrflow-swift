...
struct PointSizeReadingTests {
    ...
    private func attributed(pointSize: CGFloat) -> CFAttributedString {
    ...
    func readsThePointSize() {
        let size = FocusedFieldReader.pointSize(inAttributed: attributed(pointSize: 17))
        #expect(size == 17)
    }

    @Test("An attributed string with no font at all yields nothing rather than a wrong size.")
    func withoutAFontYieldsNothing() {
        let string = CFAttributedStringCreateMutable(nil, 0)!
        CFAttributedStringReplaceString(string, CFRange(location: 0, length: 0), "abc" as CFString)
