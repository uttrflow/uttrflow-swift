...
struct CandidateScorerSpanTests {
    ...
    func startsAfterTheSharedTokens() {
        #expect(CompletionText.firstScoredIndex(whole: [2, 10, 11, 12], typed: [2, 10]) == 2)
    }
    ...
    func retokenisedJoinStartsAtTheDivergence() {
        #expect(CompletionText.firstScoredIndex(whole: [2, 10, 30, 12], typed: [2, 10, 11]) == 2)
    }

    @Test("The first token is never scored, since nothing predicts it.")
    func firstTokenIsNeverScored() {
        #expect(CompletionText.firstScoredIndex(whole: [10, 11], typed: []) == 1)
        #expect(CompletionText.firstScoredIndex(whole: [10, 11], typed: [99]) == 1)
    }

    @Test("A candidate with nothing past what was typed has nothing to be judged on.")
    func nothingLeftToScore() {
        #expect(CompletionText.firstScoredIndex(whole: [2, 10], typed: [2, 10]) == nil)
        #expect(CompletionText.firstScoredIndex(whole: [2, 10], typed: [2, 10, 11]) == nil)
        #expect(CompletionText.firstScoredIndex(whole: [10], typed: []) == nil)
        #expect(CompletionText.firstScoredIndex(whole: [], typed: []) == nil)
    }
}
