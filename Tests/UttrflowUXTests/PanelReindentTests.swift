...
struct PanelReindentCostTests {
    ...
    func keystrokesDoNotReindent() {
        ...
        let opened = PanelFixture.panel(clips)
        #expect(Self.reindents { _ = PanelPresenter.present(opened) } == clips.count)
    ...
    @Test("reopening the panel re-indents nothing it has already asked about")
    ...
    func memoIsBounded() {
        ...
        let clips = Self.codeClips()
        for clip in clips { #expect(memo.offers(clip)) }
        #expect(memo.count <= 3)
